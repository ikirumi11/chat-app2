...
const SOLO=[
 ...
 {id:'clicker',icon:'🖱️',name:'Clicker',cat:'Solo Arcade',desc:'Click as fast as you can for 15 seconds.'},
 {id:'mathrush',icon:'➗',name:'Math Rush',cat:'Solo Arcade',desc:'Solve as many quick maths questions as possible.'},
 {id:'color',icon:'🎨',name:'Color Match',cat:'Solo Arcade',desc:'Choose the matching color as quickly as possible.'},
 {id:'maze',icon:'🧩',name:'Maze Escape',cat:'Solo Arcade',desc:'Reach the goal using the arrow keys.'}
];
const PARTY=[
 {id:'ttt3',icon:'❌',name:'Tic-Tac-Toe 3x3',cat:'Multiplayer',players:'2–3',desc:'Get 3 in a row.'},
 {id:'ttt4',icon:'❌',name:'Tic-Tac-Toe 4x4',cat:'Multiplayer',players:'2–3',desc:'Get 4 in a row.'},
 {id:'ttt5',icon:'❌',name:'Tic-Tac-Toe 5x5',cat:'Multiplayer',players:'2–3',desc:'Get 5 in a row.'},
 {id:'ttt6',icon:'❌',name:'Tic-Tac-Toe 6x6',cat:'Multiplayer',players:'2–3',desc:'Get 6 in a row.'},
 ...
 {id:'ttttournament',icon:'🏆',name:'Tic-Tac-Toe Tournament',cat:'Multiplayer',players:'2–6',desc:'Round-robin Tic-Tac-Toe.'},
 {id:'rpstournament',icon:'🏆',name:'RPS Tournament',cat:'Multiplayer',players:'2–6',desc:'Rock Paper Scissors tournament.'}
];

const style=document.createElement('style');style.id='games-ui-remake-style';style.textContent=`
.gur-overlay{position:fixed;inset:0;z-index:50000;background:#000b;display:none;align-items:center;justify-content:center;padding:14px;box-sizing:border-box}.gur-overlay.on{display:flex}.gur-box{width:min(1100px,100%);height:min(900px,95vh);display:flex;flex-direction:column;overflow:hidden;background:var(--panel,#20242b);color:var(--text,#fff);border:1px solid #fff2;border-radius:18px;box-shadow:0 25px 100px #000c}.gur-head{display:flex;align-items:center;gap:10px;padding:15px 17px;border-bottom:1px solid #fff2}.gur-head h2{margin:0;flex:1;font-size:21px}.gur-btn{border:0;border-radius:10px;padding:9px 12px;background:#fff1;color:inherit;cursor:pointer}.gur-btn:hover{background:#fff2}.gur-tools{display:grid;grid-template-columns:minmax(180px,1fr) 180px 180px;gap:8px;padding:12px;border-bottom:1px solid #fff2}.gur-input,.gur-select{box-sizing:border-box;width:100%;padding:11px;border:1px solid #fff2;border-radius:10px;background:#ffffff08;color:inherit;outline:none}.gur-list{padding:14px;overflow:auto;display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:10px;align-content:start}.gur-section{grid-column:1/-1;font-size:13px;font-weight:800;opacity:.7;margin:8px 2px -2px;text-transform:uppercase;letter-spacing:.06em}.gur-card{border:1px solid #fff2;border-radius:13px;background:#ffffff07;color:inherit;padding:15px;text-align:left;cursor:pointer;transition:.12s}.gur-card:hover{background:#ffffff12;transform:translateY(-1px)}.gur-card .gur-icon{font-size:30px}.gur-card b{display:block;margin-top:7px}.gur-card small{display:block;opacity:.62;line-height:1.35;margin-top:5px}.gur-tag{display:inline-block;margin-top:9px;padding:4px 8px;border-radius:99px;background:#fff1;font-size:11px}.gur-empty{grid-column:1/-1;text-align:center;opacity:.65;padding:45px}.gur-chat-launch{margin:8px 0;width:100%;box-sizing:border-box;padding:12px 14px;border:1px solid #fff2;border-radius:12px;background:#ffffff08;color:inherit;text-align:left;cursor:pointer}.gur-chat-launch:hover{background:#ffffff12}.gur-chat-launch b{display:block}.gur-chat-launch small{display:block;margin-top:3px;opacity:.62}@media(max-width:700px){.gur-tools{grid-template-columns:1fr 1fr}.gur-input{grid-column:1/-1}.gur-list{grid-template-columns:1fr 1fr}}@media(max-width:430px){.gur-list{grid-template-columns:1fr}}
...
function show(){overlay.classList.add('on');back.style.display='none';title.textContent='🎮 Games';render()}
function close(){overlay.classList.remove('on')}
function render(){const q=search.value.trim().toLowerCase();const pc=players.value;let data=all.filter(g=>{if(category.value!=='all'&&g.cat!==category.value)return false;if(q&&!(`${g.name} ${g.desc} ${g.cat}`).toLowerCase().includes(q))return false;if(pc!=='all'&&g.cat==='Solo Arcade'&&pc!=='1')return false;if(pc!=='all'&&g.cat==='Multiplayer'){const nums=(g.players||'').match(/\d+/g)||[];if(!nums.some(n=>+n===+pc))return false}return true});data.sort((a,b)=>a.cat.localeCompare(b.cat)||a.name.localeCompare(b.name));let html='';let last='';for(const g of data){if(g.cat!==last){last=g.cat;html+=`<div class="gur-section">${g.cat}</div>`}html+=`<button class="gur-card" data-game="${g.id}" data-solo="${g.cat==='Solo Arcade'}"><span class="gur-icon">${g.icon}</span><b>${esc(g.name)}</b><small>${esc(g.desc)}</small><span class="gur-tag">${g.cat==='Solo Arcade'?'1 player':g.players+' players'}</span></button>`}listEl.innerHTML=html||'<div class="gur-empty">No games match your search or filters.</div>';listEl.querySelectorAll('[data-game]').forEach(b=>b.onclick=()=>choose(b.dataset.game,b.dataset.solo==='true'))}
function esc(s){return String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function choose(id,solo){if(solo){openSolo(id)}else{close();const old=document.querySelector(`.game-choice[data-game="${CSS.escape(id)}"]`);if(old){old.click();return}const launch=document.querySelector('.expanded-games-launch');if(launch){launch.click();setTimeout(()=>{const x=document.querySelector(`.gbcard[data-id="${CSS.escape(id)}"]`);if(x)x.click()},350)}}}
function openSolo(id){close();const launch=document.querySelector('.gp-launch');if(!launch){alert('Solo Arcade is still loading. Please try again in a moment.');return}launch.click();const find=()=>{const card=document.querySelector(`.gp-card[data-gp="${CSS.escape(id)}"]`);if(card){card.click();return true}return false};let tries=0;const t=setInterval(()=>{if(find()||++tries>25)clearInterval(t)},80)}
search.oninput=render;category.onchange=render;players.onchange=render;overlay.querySelector('#gurClose').onclick=close;overlay.onclick=e=>{if(e.target===overlay)close()};window.addEventListener('keydown',e=>{if(e.key==='Escape'&&overlay.classList.contains('on'))close()});
window.__openGamesUI=show;
