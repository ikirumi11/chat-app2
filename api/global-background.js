export default async function handler(req, res) {
  // Use the requested Chat App 2 Supabase project.
  // Configure SUPABASE_ANON_KEY (or SUPABASE_PUBLISHABLE_KEY) in the deployment environment.
  ...
  if (!key) {
    return res.status(500).json({ error: 'Supabase key is not configured. Set SUPABASE_ANON_KEY or SUPABASE_PUBLISHABLE_KEY.' });
  }
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (finalUrl.startsWith('data:image/')) {
        ...
        if (!match) return res.status(400).json({ error: 'Unsupported uploaded image format' });
        ...
        const upload = await fetch(`${supabaseUrl}/storage/v1/object/${bucket}/${path}`, {
          method: 'POST',
          headers: { apikey: key, Authorization: `Bearer ${key}`, 'Content-Type': contentType, 'x-upsert': 'true', 'cache-control': '3600' },
          body: binary
        });
        ...
        let parsed;
        try { parsed = new URL(finalUrl); } catch (e) { return res.status(400).json({ error: 'Enter a valid image URL' }); }
      ...
      const r = await fetch(`${supabaseUrl}/rest/v1/chat_global_settings`, {
        ...
        body: JSON.stringify({ id: 1, url: finalUrl, updated_at: new Date().toISOString() })
      });
      if (!r.ok) {
