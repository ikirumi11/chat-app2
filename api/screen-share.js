export default async function handler(req,res){
  ...
  const trim=async all=>{const frames=all.filter(r=>parse(r)?.type==="frame");if(frames.length<=100)return;await Promise.all(frames.slice(100).map(remove))};
  try{
    let all=await rows();
    if(req.method==="GET"){
      const parsed=all.map(parse).filter(Boolean),control=parsed.find(x=>x.type==="control");
      if(control&&Date.now()-(Number(control.updatedAt)||new Date(control.createdAt).getTime())>5000){await Promise.all(all.map(remove));return res.status(200).json({success:true,share:null,frames:[]})}
      const frames=parsed.filter(x=>x.type==="frame").sort((a,b)=>new Date(a.createdAt)-new Date(b.createdAt));
      return res.status(200).json({success:true,share:control||null,frames:frames.slice(-100)})
    }
    if(!deviceId)return res.status(400).json({error:"Device ID is required."});
    if(req.method!=="POST")return res.status(405).json({error:"Method not allowed."});
    const action=String(body.action||"");
    if(action==="start"){
      const control=all.map(parse).find(x=>x?.type==="control");
    ...
    const control=all.map(parse).find(x=>x?.type==="control"&&String(x.id)===shareId);
    if(!control||control.deviceId!==deviceId)return res.status(409).json({success:false,error:"You are not the screen-share host."});
    if(action==="frame"){
      if(control.frozen)return res.status(204).end();
      if(typeof body.image!=="string")return res.status(400).json({error:"Frame is missing."});
      const frame={type:"frame",state:"sharing",deviceId,username:control.username,quality:control.quality,fps:control.fps,image:body.image,updatedAt:Date.now()};
      ...
      // Keep only the newest 100 frames. These are disposable screen-share data, not chat messages.
      const after=await rows();await trim(after);return res.status(204).end();
    }
    if(action==="heartbeat"||action==="freeze"){
      const next={...control,updatedAt:Date.now()};if(action==="freeze")next.frozen=Boolean(body.frozen);delete next.id;delete next.deviceId;delete next.createdAt;
