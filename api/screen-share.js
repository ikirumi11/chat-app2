export default async function handler(req,res){
  ...
  try{
    let row=await current(),s=parse(row);
    if(req.method==="GET"){
      if(s&&s.state==="sharing"&&Date.now()-(Number(s.updatedAt)||new Date(s.createdAt).getTime())>5000){return res.status(200).json({success:true,share:s})}
      // ONE database message contains the whole temporary share. The frames are inside that message.
      const frames=Array.isArray(s?.frames)?s.frames:[];
      const watch=String(req.query?.watch||"")==="1",after=Number(req.query?.after)||0;
      const now=Date.now(),target=now-1000;
      let frame=null;
      if(watch){
        frame=frames.find(x=>(Number(x.capturedAt)||0)>after&&(Number(x.capturedAt)||0)<=target)||null;
      }
      return res.status(200).json({success:true,share:s,frame,bufferedFrames:frames.filter(x=>(Number(x.capturedAt)||0)>target).length,delayMs:1000})
    }
    ...
    if(action==="frame"){
      ...
      // Keep frames inside the SAME message. Keep ~1.25 seconds so a full second is always ready.
      const keep=Math.max(10,Math.ceil((Number(s.fps)||25)*1.25));
      const frames=Array.isArray(s.frames)?s.frames:[];
      frames.push({image:body.image,capturedAt});
      s.frames=frames.filter(x=>(Number(x.capturedAt)||0)>=capturedAt-1250).slice(-keep);s.updatedAt=Date.now();
