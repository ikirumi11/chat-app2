export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==="GET"){
      ...
      return res.status(200).json({ok:true,share:{id:row.id,type:share.type,state:share.state,host:share.host,deviceId:share.deviceId,quality:share.quality,fps:share.fps,frozen:!!share.frozen,updatedAt:share.updatedAt,edited:!!row.edited},frame:watching?share.frame:null});
    }
    ...
    if(action==="start"){
      ...
      const p={type:"screen-share",state:"sharing",deviceId:device,host:String(b.username||"User").slice(0,40),quality,fps,frozen:false,startedAt:now,updatedAt:now,frame:null,frameNumber:0};
    ...
    if(action==="frame"){
      ...
      share.frame=b.image;share.updatedAt=Date.now();share.frameNumber=Number(b.frameNumber||0);await patch(row.id,share);return res.status(204).end();
    }
