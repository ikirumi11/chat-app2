export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==="GET"){
      ...
      return res.status(200).json({ok:true,share:{id:row.id,type:share.type,state:share.state,host:share.host,deviceId:share.deviceId,peerId:share.peerId,quality:share.quality,fps:share.fps,frozen:!!share.frozen,updatedAt:share.updatedAt,edited:!!row.edited}});
    }
    ...
    if(action==="start"){
      ...
      const peerId=String(b.peer_id||"").slice(0,200);if(!peerId)return res.status(400).json({ok:false,error:"Missing PeerJS ID."});
      const p={type:"screen-share",state:"sharing",deviceId:device,host:String(b.username||"User").slice(0,40),peerId,quality,fps,frozen:false,startedAt:now,updatedAt:now};
    ...
    if(action==="frame")return res.status(410).json({ok:false,error:"Frame uploads were removed. Screen video uses PeerJS WebRTC."});
