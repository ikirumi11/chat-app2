export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='DELETE'){const id=String(b.id||'').trim();if(!id)return res.status(400).json({error:'Queue item ID required.'});const r=await fetch(base+'/rest/v1/messages?id=eq.'+encodeURIComponent(id)+'&username=eq.__YOUTUBE_QUEUE__',{method:'DELETE',headers});if(!r.ok)return res.status(500).json({error:'Could not remove queue item.'});return res.status(200).json({success:true,deleted:true})}
