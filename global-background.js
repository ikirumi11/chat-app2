/* Global Chat Background + open real Settings on startup */
(() => {
  'use strict';
  ...
  async function loadServerBackground() { try { const data = await request('background'); applyBackground(data.url || ''); } catch (error) { console.warn('Global background could not be loaded:', error); } }

  function setupBackgroundControls() {
    const category = document.getElementById('globalBackgroundSetting');
    if (!category || category.dataset.globalBackgroundReady === '1') return;
    ...
    category.dataset.globalBackgroundReady = '1';
    let selected = null;
    ...
    clearBtn.addEventListener('click', async () => {
      clearBtn.disabled = true; status.textContent = 'Removing…';
      try { await request('clear_background'); applyBackground(''); preview.removeAttribute('src'); preview.style.display = 'none'; selected = null; saveBtn.disabled = true; input.value = ''; status.textContent = '✓ Global background removed for everyone.'; }
      catch (error) { status.textContent = '✕ ' + error.message; }
  ...
  function boot() {
    ...
    new MutationObserver(() => setupBackgroundControls()).observe(document.documentElement, { childList: true, subtree: true });
  }
