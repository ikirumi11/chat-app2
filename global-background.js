...
(() => {
  ...
  function applyBackground(url) {
    ...
    style.textContent = safe ? `
      ...
      .app > .header, .app > .messages, .app > .composer {
  ...
  async function request(action, body = {}) {
    ...
    if (!response.ok || data.ok === false) {
      throw new Error(data.error || data.message || `Background request failed (${response.status})`);
    }
      ...
      reader.onload = () => {
        ...
        img.onload = () => {
          ...
          if (data.length > MAX_BYTES) {
            reject(new Error('Image is still too large. Choose a smaller image.'));
            return;
          }
  ...
  function openRealSettingsOnStart() {
    ...
    setTimeout(() => {
      // Open the existing REAL Settings panel exactly as it is.
